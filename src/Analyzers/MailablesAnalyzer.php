...
class MailablesAnalyzer implements AnalyzerInterface
{
    ...
    {
        return function_exists('app_path') ? [app_path('Mail')] : [];
    }
    ...
    {
        return 'Finds mailable classes that are never queued or sent via Mail.';
    }
    ...
    public function analyze(): array
    {
        ...
        foreach (PhpFilesUnderScanPaths::eachUniqueRealPath($this->scanner, $this->scanPaths, $this->pathExclude) as $path) {
            ...
            if ($kinds === null || ! ClassKindClassifier::hasKind($kinds, ClassKindClassifier::KIND_MAIL)) {
                continue;
            }
        ...
        $v       = new MailableUsageVisitor($targets);
...
final class MailableUsageVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node): ?int
    {
        ...
        if ($node instanceof StaticCall && $node->name instanceof Identifier) {
            ...
            if (in_array($m, ['send', 'queue', 'later'], true) && $this->isMailFacade($node->class)) {
    ...
    private function isMailFacade(?Node $class): bool
    {
        if ($class instanceof Name) {
            return $class->getLast() === 'Mail';
        }
        if ($class instanceof FullyQualified) {
            return str_ends_with($class->toString(), '\\Facades\\Mail');
        }

        return false;
    }

    private function mark(?Node $class): void
    {
        if ($class instanceof FullyQualified) {
