...
class NotificationsAnalyzer implements AnalyzerInterface
{
    ...
    {
        return function_exists('app_path') ? [app_path('Notifications')] : [];
    }
    ...
    {
        return 'Finds notification classes that are never sent.';
    }
    ...
    public function analyze(): array
    {
        ...
        foreach (PhpFilesUnderScanPaths::eachUniqueRealPath($this->scanner, $this->scanPaths, $this->pathExclude) as $path) {
            ...
            if ($kinds === null || ! ClassKindClassifier::hasKind($kinds, ClassKindClassifier::KIND_NOTIFICATION)) {
                continue;
            }
        ...
        $v       = new NotificationSendVisitor($targets);
...
final class NotificationSendVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node): ?int
    {
        if ($node instanceof MethodCall && $node->name instanceof Identifier) {
            if (strtolower($node->name->name) === 'notify') {
                foreach ($node->args as $a) {
                    $this->maybeNew($a->value ?? null);
                }
            }
        }
        ...
        if ($node instanceof StaticCall && $node->name instanceof Identifier) {
            ...
            if (in_array($m, ['send', 'sendnow'], true) && $this->isNotificationFacade($node->class)) {
                foreach ($node->args as $a) {
                    $this->maybeNew($a->value ?? null);
                    $this->maybeClassConst($a->value ?? null);
                }
            }
        }
    ...
    private function maybeNew(?Node $v): void
    {
    ...
    private function maybeClassConst(?Node $v): void
    {
    ...
    private function isNotificationFacade(?Node $class): bool
    {
        if ($class instanceof Name) {
            return $class->getLast() === 'Notification';
        }
        if ($class instanceof FullyQualified) {
            return str_ends_with($class->toString(), '\\Facades\\Notification');
        }
