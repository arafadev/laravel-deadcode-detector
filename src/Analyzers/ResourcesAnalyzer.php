...
class ResourcesAnalyzer implements AnalyzerInterface
{
    ...
    {
        return function_exists('app_path') ? [app_path('Http/Resources')] : [];
    }
    ...
    {
        return 'Finds JsonResource / ResourceCollection classes that are never made or instantiated.';
    }
    ...
    public function analyze(): array
    {
        ...
        foreach (PhpFilesUnderScanPaths::eachUniqueRealPath($this->scanner, $this->scanPaths, $this->pathExclude) as $path) {
            ...
            if ($kinds === null || ! ClassKindClassifier::hasKind($kinds, ClassKindClassifier::KIND_RESOURCE)) {
                continue;
            }
        ...
        $used    = $this->collectUsedResourceNorms($targets);
     ...
     */
    private function collectUsedResourceNorms(array $targets): array
    {
        $v = new JsonResourceUsageVisitor($targets);
...
final class JsonResourceUsageVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node): ?int
    {
        ...
        if ($node instanceof StaticCall && $node->name instanceof Identifier) {
            ...
            if (in_array($m, ['make', 'collection'], true)) {
                $this->markClass($node->class);
            }
        }
