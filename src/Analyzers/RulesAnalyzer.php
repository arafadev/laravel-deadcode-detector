...
class RulesAnalyzer implements AnalyzerInterface
{
    ...
    {
        return function_exists('app_path') ? [app_path('Rules')] : [];
    }
    ...
    {
        return 'Finds validation Rule classes that are never instantiated.';
    }
    ...
    public function analyze(): array
    {
        ...
        foreach (PhpFilesUnderScanPaths::eachUniqueRealPath($this->scanner, $this->scanPaths, $this->pathExclude) as $path) {
            ...
            if ($kinds === null || ! ClassKindClassifier::hasKind($kinds, ClassKindClassifier::KIND_RULE)) {
                continue;
            }
        ...
        $v = new RuleInstantiationVisitor($targets);
...
final class RuleInstantiationVisitor extends NodeVisitorAbstract
{
